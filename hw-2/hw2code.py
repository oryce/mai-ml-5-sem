...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    n_ones_total = np.sum(target_vector)
    n_zeros_total = n_total - n_ones_total
    ...
    for threshold in thresholds:
        ...
        n_ones_left = np.sum(target_vector[left_mask])
    ...
    def _fit_node(self, sub_X, sub_y, node, depth=0):
        ...
        for feature in range(sub_X.shape[1]):
            ...
            if self._min_samples_leaf is not None:
                left_mask = feature_vector < threshold
                if np.sum(left_mask) < self._min_samples_leaf or np.sum(~left_mask) < self._min_samples_leaf:
                    continue
